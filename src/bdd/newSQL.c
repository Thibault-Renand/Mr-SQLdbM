...
SQL_QUERY* init_query() {
    ...
    query->alter_table_columns_count=0;
    query->alter_table_definitions_count=0;
...
void add_alter_table(SQL_QUERY* query, const char* operation, const char* column, const char* definition) {
    add_field(&query->alter_table_operations, &query->alter_table_count, operation);
    add_field(&query->alter_table_columns, &query->alter_table_columns_count, column); // Separate count
    add_field(&query->alter_table_definitions, &query->alter_table_definitions_count, definition); // Separate count
}
void print_query(const SQL_QUERY* query) {
    ...
    // SECTION ALTER TABLE
    if (query->alter_table_count > 0) {
        for (int i = 0; i < query->from_count; i++) {
            printf("ALTER_TABLE[%d] : %s\n", i, query->from_tables[i]);
        }
        for (int i = 0; i < query->alter_table_count; i++) {
            printf("ALTER_OPERATION[%d] : %s\n", i, query->alter_table_operations[i]);
            printf("ALTER_COLUMN[%d] : %s\n", i, query->alter_table_columns[i]);
            printf("ALTER_DEFINITION[%d] : %s\n", i, query->alter_table_definitions[i]);
        }
    }
...
void free_query(SQL_QUERY* query) {
    ...
    for (int i = 0; i < query->create_table_count; i++) {
        free(query->create_table_definitions[i]);
    }
    free(query->create_table_definitions);

    // Free ALTER TABLE memory
    for (int i = 0; i < query->alter_table_count; i++) {
        free(query->alter_table_operations[i]);
        free(query->alter_table_columns[i]);
    }
    free(query->alter_table_operations);
    free(query->alter_table_columns);
...
void process_alter_table_section(const char* query_str, SQL_QUERY* query) {
    if (!query_str || !query) {
        fprintf(stderr, "Error: NULL input to process_alter_table_section.\n");
        return;
    }

    const char* table_start = strstr(query_str, "ALTER TABLE");
    if (!table_start) {
        fprintf(stderr, "Error: ALTER TABLE syntax invalid.\n");
        return;
    }

    // Extract the table name
    char table_name[128] = {0};
    sscanf(table_start + 12, "%127s", table_name);
    add_from(query, table_name);

    // Declare column_name here
    char column_name[256] = {0};
    char column_definition[256] = {0};

    // Extract operations
    const char* operation_start = strstr(query_str, "ADD");
    if (operation_start) {
        // Ensure memory is allocated for the column
        query->alter_table_columns = realloc(query->alter_table_columns, (query->alter_table_count + 1) * sizeof(char*));
        if (!query->alter_table_columns) {
            perror("Memory allocation error for alter_table_columns");
            exit(EXIT_FAILURE);
        }

        // Store the column name in the array
        query->alter_table_columns[query->alter_table_count] = strdup(column_name);

        printf("Debug: operation_start = '%s'\n", operation_start);
        int parsed = sscanf(operation_start, "ADD %255s %255[^;]", column_name, column_definition);
        if (parsed != 2) {
            fprintf(stderr, "Error: Failed to parse ALTER TABLE ADD operation. Parsed: %d\n", parsed);
            return;
        }

        // Debugging output
        printf("Debug: Adding column '%s' with definition '%s'\n", column_name, column_definition);

        add_alter_table(query, "ADD", column_name, column_definition);
    }

    operation_start = strstr(query_str, "DROP");
    if (operation_start) {
        char operation[256] = {0};
        sscanf(operation_start, "DROP %255s", operation);
        add_field(&query->alter_table_operations, &query->alter_table_count, "DROP");
        add_field(&query->alter_table_columns, &query->alter_table_count, operation);
    }

    operation_start = strstr(query_str, "MODIFY");
    if (operation_start) {
        char operation[256] = {0};
        sscanf(operation_start, "MODIFY %255s %255[^;]", operation, column_definition);
        add_field(&query->alter_table_operations, &query->alter_table_count, "MODIFY");
        add_field(&query->alter_table_columns, &query->alter_table_count, operation);
        add_field(&query->alter_table_definitions, &query->alter_table_count, column_definition);
    }
}
void parse_query(const char* query_str, SQL_QUERY* query) {
    if (strstr(query_str, "ALTER TABLE")) {
        process_alter_table_section(query_str, query);
    } else if (strstr(query_str, "DELETE FROM")) {
